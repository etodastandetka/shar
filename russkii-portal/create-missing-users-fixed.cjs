...
// Правильная функция хеширования
...
console.log('🔧 СОЗДАНИЕ НЕДОСТАЮЩИХ ПОЛЬЗОВАТЕЛЕЙ (ИСПРАВЛЕННАЯ ВЕРСИЯ)');
...
const knownRegistrations = [
  {
    ...
    password: 'DefaultPass123' // Безопасный дефолтный пароль
  },
  {
    phone: '+79615163632',
    ...
    const createUser = (index) => {
      if (index >= toCreate.length) {
        ...
        if (created > 0) {
          ...
          console.log('🔑 Дефолтный пароль для всех: DefaultPass123');
      ...
      const hashedPassword = hashPassword(reg.password); // Правильное хеширование!
      
      console.log(`\n${index + 1}. Создание: ${reg.email}`);
      console.log(`   📞 Телефон: ${reg.phone}`);
      console.log(`   🆔 ID: ${userId}`);
      console.log(`   🔒 Хеш пароля: ${hashedPassword.substring(0, 20)}...`);
      ...
      db.run(sql, [userId, reg.email, hashedPassword, reg.phone, reg.token], function(insertErr) {
        if (insertErr) {
          ...
          console.log(`   ✅ Создан успешно с правильным хешированием`);
